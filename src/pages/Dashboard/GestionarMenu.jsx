  ...
  return (
    <div className="admin-container">
      <Card>
        ...
        <Table columns={columns} dataSource={menus} rowKey="id" loading={loading} pagination={{ pageSize: 10 }} scroll={{ x: 800 }} />
      </Card>
      ...
      <Modal title={editingMenu ? "Editar Menú" : "Nuevo Menú"} open={modalVisible} onCancel={handleCloseModal} footer={null} width={500}>
        <Form form={form} layout="vertical" onFinish={handleSubmit}>
          ...
          <Form.Item name="titulo" label="Título" rules={[{ required: true }]}>
            <Input prefix={<AppstoreOutlined />} placeholder="Ej: Sobre Nosotros" onChange={handleTitleChange} />
          </Form.Item>
          
          <Form.Item name="ruta" label="Ruta (Auto-generada)" rules={[{ required: true }]}>
            <Input prefix={<LinkOutlined />} placeholder="Ej: /sobre-nosotros" />
          </Form.Item>
          
          <Form.Item name="icono" label="Seleccionar Icono">
            <Select 
                ...
                return (
                  <Option value={iconName} key={iconName} label={iconName}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      <span style={{ fontSize: '22px', color: '#333' }}>{React.createElement(IconComp)}</span>
